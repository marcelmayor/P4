...
compute_mfcc() {
    for filename in $(cat $lists/class/all.train $lists/class/all.test); do
        mkdir -p `dirname $w/$FEAT/$filename.$FEAT`
          EXEC="wav2mfcc 16 20 22 $db/$filename.wav $w/$FEAT/$filename.$FEAT"
        echo $EXEC && $EXEC || exit 1
    done
}
...
for cmd in $*; do
   ...
   if [[ $cmd == train ]]; then
       ...
       for dir in $db/BLOCK*/SES* ; do
           ...
           # gmm_train -v 1 -T 1e-6 -n 40 -m 40 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train || exit 1
            gmm_train -i 1 -v 1 -T 1e-6 -N 250 -m 40 -n 100 -t 1e-6 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train || exit 1
           echo
       done
   elif [[ $cmd == test ]]; then
       ...
       # gmm_train -v 1 -T 1e-6 -n 40 -m 40 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$world.gmm $lists/verif/$world.train || exit 1
       # DONE Realizado
       gmm_train -i 1 -n 40 -t 1e-6 -v 1 -T 1e-3 -N 50 -m 40 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$world.gmm $lists/verif/$world.train || exit 1
       ...
       # compute_mfcc $db_test $lists/final/class.test
       ...
       #echo "To be implemented ..."
       # \DONE Realizado
       # compute_mfcc $db_test $lists/final/verif.test
       ...
       perl -ane 'print "$F[0]\t$F[1]\t";
                  if ($F[2] > 0.540715836899197) {print "1\n"}
